        ...
        if ~isempty(testValData)
            ...
            if contaminationFraction > 0
                ...
                for i = 1:size(XTestValCell, 1)
                    [anomalyScores_tmp, ~, labelsTestVal_tmp] = detectWithDNN(options, Mdl, XTestValCell{i, 1}, YTestValCell{i, 1}, labelsTestValCell{i, 1}, false);
                ...
                if options.hyperparameters.training.ratioTrainVal.value ~= 1
                    ...
                    [anomalyScoresVal, ~, ~] = detectWithDNN(options, Mdl, XVal, YVal, zeros(size(YVal{1, 1}, 1), 1), false);
